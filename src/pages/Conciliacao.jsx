...
function lerHistorico(h) {
  ...
  // NF pela regra ÚNICA (ignora competência "07/2026", CNPJ/CPF e ACUM.). `nfm` fica só para
  // recortar o marcador do NOME abaixo (não é mais a fonte do número da nota).
...
function Detalhe({ conta, tipoCta, reg, compId, empresaId, usuario, competencia, ajuste = null, getCompetenciaId, onSalvarConf, onMudou, onVoltar }) {
  ...
  async function vincularLote(lines, nomeAlvo) {
    ...
    for (const l of razaoLinhas) {
      ...
      // ÚLTIMA ORDEM VENCE (regra do usuário): mesmo que você tenha marcado "Não é o mesmo" antes,
      // ao mandar JUNTAR agora, tira TODOS os nomes selecionados do "isolado" — inclusive o alvo —
      // para o "é o mesmo" de agora ganhar do "não era" de antes.
      if (k) iso.delete(k)
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'grid', placeItems: 'center', padding: 20, zIndex: 95 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 'min(560px,96vw)', background: theme.card, border: `0.5px solid ${theme.cb}`, borderRadius: 14, padding: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div style={{ minWidth: 0 }}><label style={{ fontSize: 12, color: theme.sub }}>Data</label><input className="input" type="date" value={f.data} onChange={e => set('data', e.target.value)} style={{ width: '100%', boxSizing: 'border-box' }} /></div>
          <div style={{ minWidth: 0 }}><label style={{ fontSize: 12, color: theme.sub }}>Valor</label><input className="input" type="number" step="0.01" value={f.valor} onChange={e => set('valor', e.target.value)} placeholder="0,00" style={{ width: '100%', boxSizing: 'border-box' }} /></div>
          <div style={{ minWidth: 0 }}><label style={{ fontSize: 12, color: theme.sub }}>Conta débito (F4)</label><CampoConta value={f.conta_debito} onChange={v => set('conta_debito', v)} plano={plano} /></div>
          <div style={{ minWidth: 0 }}><label style={{ fontSize: 12, color: theme.sub }}>Conta crédito (F4)</label><CampoConta value={f.conta_credito} onChange={v => set('conta_credito', v)} plano={plano} /></div>
          <div style={{ gridColumn: 'span 2', minWidth: 0 }}><label style={{ fontSize: 12, color: theme.sub }}>Histórico</label><input className="input" value={f.historico} onChange={e => set('historico', e.target.value)} placeholder="Descrição do lançamento" style={{ width: '100%', boxSizing: 'border-box' }} /></div>
        </div>
