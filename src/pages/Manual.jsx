...
<section id="m-base">
  ...
  <div class="m-rule"><span class="k">Contas de lucros e prejuízos</span> O card <b>Contas de lucros e prejuízos</b> amarra o <b>resultado do exercício</b> a uma conta do plano no <b>balanço</b>. Você indica <b>duas contas analíticas</b> do Patrimônio Líquido: a de <b>lucro</b> (usada quando o resultado fica <b>credor</b>) e a de <b>prejuízo</b> (quando fica <b>devedor</b>). No balanço, o resultado deixa de aparecer solto fora do PL e passa a <b>somar a conta escolhida</b>, subindo pela estrutura (analítica → sintéticas → PL). É só efeito de balanço (<b>virtual</b>): <b>nada é lançado</b> no razão nem exportado para o Domínio. Sem amarração, o balanço segue mostrando o resultado como <b>linha própria</b> no PL, como antes.</div>
</section>
...
<section id="m-fechamentos">
  ...
  <div class="m-rule"><span class="k">Conferir o balancete (Excel ou PDF)</span> No bloco <b>“Arquivo para importação no Domínio”</b>: <b>Importar balancete (Excel/PDF)</b> sobe o balancete exportado do Domínio, em <b>Excel</b> (.xlsx/.xls/.csv) <b>ou PDF</b>, e <b>confere conta a conta</b> com o que a plataforma calculou (razão vivo + correções). Casa cada conta por <b>código</b>, por <b>classificação</b> ou pelo <b>nome</b> (quando o código do PDF não bate). Se <b>bater</b>, libera o encerramento; se <b>não bater</b>, mostra uma tabela com <b>exatamente as contas divergentes</b> (Conciliação × Balancete × Diferença) para você achar e corrigir. O resultado <b>fica guardado na tela</b> (não some ao sair): você corrige o que apontou e <b>reimporta</b> para atualizar — não precisa reimportar toda hora. O <b>arquivo importado fica salvo</b> e dá para <b>Extrair</b> (baixar) de novo, igual à carga de saldos iniciais. <b>Gerar arquivo</b> gera o arquivo de lançamentos (estornos/correções) para subir no Domínio. <i>(A leitura de PDF é por heurística do layout do Domínio; se um PDF específico não for reconhecido, use o Excel.)</i></div>
  ...
  <div class="m-rule"><span class="k">Balanço Patrimonial (níveis + Domínio)</span> O card <b>Balanço</b> traz a hierarquia (grupo → sintéticas → analíticas) em duas colunas (Ativo | Passivo + PL), com <b>seletor de Nível</b> (padrão 4, igual ao balancete) e botão <b>Gerar balanço (padrão Domínio)</b> em PDF. O <b>resultado do período</b> entra no PL como <b>Lucros/Prejuízo do Exercício</b>, fechando <b>Ativo = Passivo + PL</b>. Se o cliente amarrar as <b>Contas de lucros e prejuízos</b> (Base de Informações), o resultado é <b>alocado nessa conta analítica</b> e soma dentro da estrutura do PL, em vez de aparecer como linha solta.</div>
...
<section id="m-conciliacao">
  ...
  <div class="m-rule"><span class="k">Apelido só vale entre nomes do mesmo cliente</span> Quando você <b>conecta/confirma</b> linhas, o sistema aprende um <b>apelido</b> (unifica os nomes escritos diferentes num só). Esse apelido agora só é aplicado se o nome final for <b>do mesmo cliente</b> (compartilha uma palavra distintiva). Assim, se num estado antigo o sistema tinha juntado por engano <b>clientes totalmente diferentes</b> num grupo e isso virou apelido, ele <b>deixa de forçar</b> esse agrupamento depois que a leitura passou a extrair o nome certo — cada cliente volta para o seu grupo <b>sozinho</b>, sem precisar desfazer nada à mão.</div>
  ...
  <div class="m-rule"><span class="k">Só o que zera fica em “Conciliados”</span> Em contas de cliente/fornecedor, um grupo só entra em <b>“Conciliados (o que zerou)”</b> quando <b>soma zero</b> (título + baixa se compensam) e tudo está tratado. Um <b>saldo inicial sem o par</b> (a baixa vem no mês seguinte) <b>fica no em aberto</b> até você linkar — nada que não zere sobe pra planilha de conciliados.</div>
  <div class="m-rule"><span class="k">Você manda: corrigir e linkar</span> <b>Corrigir o nome</b> de uma linha a <b>tira da união</b> e leva pro grupo do nome corrigido (a correção vence o vínculo). <b>Linkar</b> (Conectar) dois valores que se anulam <b>concilia</b> os dois — mesmo com nomes diferentes — e, se você já tinha corrigido uma das linhas, o link <b>prevalece</b> (é a ação mais recente). Nada disso é automático: é só o que você faz. <i>Links feitos em versões antigas podem precisar ser refeitos uma vez.</i></div>
  <div class="m-rule"><span class="k">O “Saldo anterior” também se marca e se linka</span> A linha <b>“Saldo anterior”</b> (implantação, sem NF própria) tem <b>checkbox</b> como qualquer outra: dá para <b>marcá-la junto</b> com o pagamento do mês e clicar em <b>Conectar (baixar)</b>. Quando o par <b>zera</b>, os dois vão para <b>“Conciliados”</b> — inclusive quando o saldo anterior já estava <b>conferido</b>. Antes ela não podia ser selecionada, então um par que zerava ficava “preso” no em aberto; agora baixa direto.</div>
  <div class="m-rule"><span class="k">O par que você conecta baixa sozinho (não precisa o nome inteiro zerar)</span> Quando você <b>marca duas (ou mais) linhas que zeram entre si</b> e clica em <b>Conectar (baixar)</b> ou <b>Vincular</b>, <b>esse par</b> vai para “Conciliados” — <b>mesmo que o mesmo fornecedor tenha outras linhas ainda abertas</b>. Ex.: ATTENTIVE com 3 lançamentos — você concilia o par (saldo anterior + pagamento da NF 44049) e a 3ª linha (outra NF) segue em aberto normalmente. É a régua: “o que eu conecto e zera, baixa; o resto continua compondo o saldo”.</div>
...
<section id="m-relatorios">
  ...
  <div class="m-rule"><span class="k">Não regenera à toa (cache)</span> Depois de gerar, ao <b>sair e voltar</b> para a tela o relatório mostra a <b>última versão na hora</b>, sem remontar. Vale para <b>todos</b>: Balancete, DRE, Balanço, DFC, Comparativo, Justificativas/Correções, Pendências, Banco × Resultado, Distribuição, além do Cockpit e do Book. Só reprocessa se algum dado do fechamento <b>mudou de fato</b> — a plataforma confere um <b>carimbo barato</b> (contagem + data da última alteração de lançamentos, razão, balancete, conciliação, justificativas, apelidos e comentários). Mudou → recalcula sozinho; não mudou → versão pronta. O <b>Comparativo</b> é guardado por <b>ano</b> (ele lê os 12 meses), então trocar de mês dentro do mesmo ano também abre na hora. Vale enquanto a aba estiver aberta; um <b>refresh</b> (F5) limpa o cache e monta de novo.</div>
...
<section id="m-demonstracoes">
  ...
  <div class="m-rule"><span class="k">O que incluir</span> Marque os blocos: <b>Cockpit</b> (o <b>painel completo</b>, igual ao Cockpit dos Relatórios — Resultado do período, Comparativo resumo, Balanço em cards com <b>Resultado acumulado</b>, <b>Distribuição de lucros no mês</b> e <b>ATA saldo a pagar</b>, Financeiro por banco, Impostos, Principais clientes e Índices), <b>DRE</b>, <b>Balancete</b> (completo), <b>Balanço Patrimonial</b>, <b>DFC</b> (fluxo de caixa, método indireto) e <b>Comparativo de Movimento</b> (folha em paisagem). O relatório sai sempre nesta sequência: Capa/desempenho → Cockpit → DRE → Balancete → Balanço → DFC → Comparativo.</div>
  <div class="m-rule"><span class="k">Modelo do relatório</span> Escolha o <b>visual</b> das demonstrações: <b>Sistema</b> (padrão — tabelas limpas Attentive, Cambria/Calibri) ou <b>Domínio</b> (fac-símile preto/branco, Arial, igual ao Domínio). Muda <b>só a aparência</b> de DRE, Balancete, Balanço e Comparativo — os dados e a estrutura são os mesmos. Capa e Cockpit ficam sempre no visual Attentive.</div>
  <div class="m-rule"><span class="k">Nível e data do Comparativo</span> O <b>Nível de detalhe</b> vale só para o <b>Balanço</b> (nível 1 = só grupos; Tudo = até as analíticas). O <b>Comparativo de Movimento</b> tem <b>data própria</b> (mês de/até) para as colunas, independente do período do relatório.</div>
  <div class="m-rule"><span class="k">Comparativo na estrutura da classificação</span> O <b>Comparativo de Movimento</b> do demonstrativo sai no <b>mesmo padrão</b> do Comparativo dos Relatórios: ordenado pela <b>classificação</b> (sintéticas + analíticas, indentado por nível), com <b>Conta · Classificação · Nome</b> e cada valor com a <b>natureza D/C</b> (devedor/credor).</div>
  <div class="m-rule"><span class="k">Lucro em verde, prejuízo em vermelho</span> Para facilitar a leitura do cliente, as linhas de <b>resultado</b> saem coloridas no <b>DRE</b> e no <b>Comparativo</b> (tela e PDF, relatório e demonstrativo): <b>lucro</b> (crédito) em <b>verde</b>, <b>prejuízo</b> (débito) em <b>vermelho</b>.</div>
  <div class="m-rule"><span class="k">Balanço fecha com o resultado no PL</span> O <b>Balanço</b> (tanto o card em Relatórios quanto o das Demonstrações) usa o <b>mesmo cálculo</b> (<span class="m-code">apurarBalanco</span>): separa Ativo e Passivo+PL pela <b>classificação</b> (grupo 1 e 2) e joga o <b>resultado acumulado do ano</b> — que vem do <b>Comparativo de Movimento</b> (a linha RESULTADO DO EXERCÍCIO) — no PL como <b>Lucros do Exercício</b> (se positivo) ou <b>Prejuízo do Exercício</b> (se negativo), somando no <b>total do Passivo + PL</b>. Se esse resultado não fechar com o patrimônio, o relatório <b>acusa a diferença</b> (Ativo × Passivo + PL) para conferência, em vez de esconder. Se o cliente amarrar as <b>Contas de lucros e prejuízos</b> (Base de Informações), esse resultado é <b>alocado na conta analítica</b> do PL e sobe pela estrutura, em vez de linha solta — via a fonte única <span class="m-code">prepararBalanco</span>, que os dois (Relatórios e Demonstrações) compartilham.</div>
