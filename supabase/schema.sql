...
create table if not exists public.cargas_cadastro (
  id          uuid primary key default gen_random_uuid(),
  cliente_id  uuid not null references public.clientes(id) on delete cascade,
  tipo        text not null check (tipo in ('plano','depara','apelidos','bancoresult','financeiro','contas_bancarias','memoria_financeira','centro_custo','lalur','conciliacao_nomes','cartoes','memoria_cartao')),
